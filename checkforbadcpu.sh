#!/bin/sh
if [ ! -d /var/tmp/badcpu ]; then
  mkdir /var/tmp/badcpu
fi
cd /var/tmp/badcpu
rm ./*.out ./all
touch all
...
for i in $SERVERS; do
        (ssh -q $i '(echo -n "'$i' " ;echo -n `hpasmcli -s "Show server" |egrep "Processor:|Status" |egrep -B1 "degraded|error|fail"`)' 2>&1 &) > /var/tmp/badcpu/$i.out
done
sleep 300
